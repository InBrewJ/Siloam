...
def createDAECamerasV2(x, y, scene, context, daeModel=None):    
    coords = []
    global daeCameraDimensions
...
def setupDAEEnv(x, y, version="V2", radius=0, ssModel=False):
    ...
    if bpy.data.objects.get('SketchUp') is None and ssModel == False:
        ...
        if version is "V1":
            ...
            if ssModel == True:
                createDAECamerasV2(x, y, bpy.context.screen.scene, context)
...
def renderSL(ssModel=False):
    ...
    if daeRenderPath == []:
        ...
        print("Rendering views of a model captured from SiloamSee. Make sure it's clean!")  
...
#
# This function also optionally imports OBJs generated by SiloamSee. Refactoring is probably
# needed to make naming etc make more sense

def importDAE(daePath, ssModel=False):
    global daeRenderPath
    global daeModelType
    ...
    # The following parses the filepath of the model which must
    # adhere to a very strict format
    
    if tempPath[-1] == "model.dae" or tempPath[-1] == "map.obj":
    ...
    if ssModel == False:
        ...
        bpy.ops.import_scene.obj(filepath=daePath, axis_forward='-Z', axis_up='Y', filter_glob="*.obj;*.mtl")
